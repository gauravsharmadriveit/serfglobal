<?php include('style.php'); ?>
<?php include('header.php'); ?>
<section>
   <div class="rs-breadcrumbs img7">
      <div class="container">
         <div class="breadcrumbs-inner">
            <h1 class="page-title">
               Event Management Team           
            </h1>
         </div>
      </div>
   </div>
</section>
<section class="comman_section clearfix">
   <div class="container clearfix">
      <div class="row clearfix">
         <div class="col-md-12">
            <div class="event_tabs">
               <ul class="nav nav-tabs nav-pills nav-fill" id="myTab">
                  <li class="nav-item">
                     <a href="#mentor" class="nav-link active" data-bs-toggle="tab">Mentor</a>
                  </li>
                  <li class="nav-item">
                     <a href="#chief-advisor" class="nav-link" data-bs-toggle="tab">Chief Advisor</a>
                  </li>
                  <li class="nav-item">
                     <a href="#advisor" class="nav-link" data-bs-toggle="tab">Advisor</a>
                  </li>
                  <li class="nav-item">
                     <a href="#executive-committee" class="nav-link" data-bs-toggle="tab">Executive Committee</a>
                  </li>
                  <li class="nav-item">
                     <a href="#working-committee" class="nav-link" data-bs-toggle="tab">Working Committee</a>
                  </li>
               </ul>
               <div class="tab-content">
                  <div class="tab-pane fade show active" id="mentor">
                     <div class="row justify-content-center align-items-center clearfix">
                        <div class="col-sm-12 col-md-3 col-lg-3">
                           <div class="our-team mt-3">
                              <div class="pic">
                                 <img src="assets/img/team/dranirudha-pradhan.jpg">
                              </div>
                              <h3 class="title">Prof.(Dr.) Anirudh Pradhan</h3>
                              <span class="post">Director, CCASS, GLA University </span>
                              <span class="d-block country_name">India</span>                               
                           </div>
                        </div>
                     </div>
                  </div>
                  <div class="tab-pane fade" id="chief-advisor">
                     <div class="row justify-content-center align-items-center clearfix">
                        <div class="col-sm-12 col-md-3 col-lg-3">
                           <div class="our-team mt-3">
                              <div class="pic">
                                 <img src="assets/img/team/user.jpg">
                              </div>
                              <h3 class="title">Prof.(Dr.) Alex Khang</h3>
                              <span class="post">Data Scientist </span>
                              <span class="d-block country_name">Vietnam</span>                               
                           </div>
                        </div>
                        <div class="col-sm-12 col-md-3 col-lg-3">
                           <div class="our-team mt-3">
                              <div class="pic">
                                 <img src="assets/img/team/user.jpg">
                              </div>
                              <h3 class="title">Prof.(Dr.) Olha Prekopenko,</h3>
                              <span class="post">Estonian Entrepreneurship University of Applied Sciences </span>
                              <span class="d-block country_name">Estonia</span>                               
                           </div>
                        </div>
                     </div>
                  </div>
                  <div class="tab-pane fade" id="advisor">
                     <div class="row justify-content-center align-items-center clearfix">
                        <div class="col-sm-12 col-md-3 col-lg-3">
                           <div class="our-team mt-3">
                              <div class="pic">
                                 <img src="assets/img/team/user.jpg">
                              </div>
                              <h3 class="title">Dr. Shyamala Krishannair</h3>
                              <span class="post">Zululand University  </span>
                              <span class="d-block country_name">South Africa</span>                               
                           </div>
                        </div>
                        <div class="col-sm-12 col-md-3 col-lg-3">
                           <div class="our-team mt-3">
                              <div class="pic">
                                 <img src="assets/img/team/user.jpg">
                              </div>
                              <h3 class="title">Prof.(Dr.) Manoj Dubey</h3>
                              <span class="post">IPS Academy</span>
                              <span class="d-block country_name">Indore, India</span>                               
                           </div>
                        </div>
                        <div class="col-sm-12 col-md-3 col-lg-3">
                           <div class="our-team mt-3">
                              <div class="pic">
                                 <img src="assets/img/team/user.jpg">
                              </div>
                              <h3 class="title">Prof.(Dr.) Vivek Raich</h3>
                              <span class="post">PG Holkar College </span>
                              <span class="d-block country_name">Indore, India</span>                               
                           </div>
                        </div>
                     </div>
                  </div>
                  <div class="tab-pane fade" id="executive-committee">
                     <div class="row justify-content-center align-items-center clearfix">
                        <div class="col-md-12">
                           <div class="title_spekaer">
                              <h2>President</h2>
                           </div>
                        </div>
                        <div class="col-sm-12 col-md-3 col-lg-3">
                           <div class="our-team mt-3">
                              <div class="pic">
                                 <img src="assets/img/team/dr-sanjeev-sir.jpg">
                              </div>
                              <h3 class="title">Prof.(Dr.) Sanjeev Kumar</h3>
                              <span class="d-block country_name">India</span>                               
                           </div>
                        </div>
                        <div class="col-sm-12 col-md-3 col-lg-3">
                           <div class="our-team mt-3">
                              <div class="pic">
                                 <img src="assets/img/team/nevine.jpg">
                              </div>
                              <h3 class="title">Prof.(Dr.) Nevine Makram Labib</h3>
                              <span class="d-block country_name">Egypt</span>                               
                           </div>
                        </div>
                        <div class="col-md-12">
                           <div class="title_spekaer">
                              <h2>Vice President</h2>
                           </div>
                        </div>
                        <div class="col-sm-12 col-md-3 col-lg-3">
                           <div class="our-team mt-3">
                              <div class="pic">
                                 <img src="assets/img/team/user.jpg">
                              </div>
                              <h3 class="title">Prof.(Dr.) Valentina E Balas</h3>
                              <span class="d-block country_name">Romania</span>                               
                           </div>
                        </div>
                        <div class="col-sm-12 col-md-3 col-lg-3">
                           <div class="our-team mt-3">
                              <div class="pic">
                                 <img src="assets/img/team/dr-dalia-mam.jpg">
                              </div>
                              <h3 class="title">Prof.(Dr.) Daliya Younis</h3>
                              <span class="d-block country_name">Egypt</span>                               
                           </div>
                        </div>
                        <div class="col-sm-12 col-md-3 col-lg-3">
                           <div class="our-team mt-3">
                              <div class="pic">
                                 <img src="assets/img/team/user.jpg">
                              </div>
                              <h3 class="title">Prof.(Dr.) Shyamrao V Gumaste</h3>
                              <span class="post">MET’s Institute of Engineering</span>                                 
                              <span class="d-block country_name">Nashik, India</span>                               
                           </div>
                        </div>
                        <div class="col-sm-12 col-md-3 col-lg-3">
                           <div class="our-team mt-3">
                              <div class="pic">
                                 <img src="assets/img/team/user.jpg">
                              </div>
                              <h3 class="title">Prof.(Dr.) Farook Rahaman</h3>
                              <span class="d-block country_name">India</span>                               
                           </div>
                        </div>
                        <div class="col-sm-12 col-md-3 col-lg-3">
                           <div class="our-team mt-3">
                              <div class="pic">
                                 <img src="assets/img/team/user.jpg">
                              </div>
                              <h3 class="title">Prof.(Dr.) Shashi Sekhar Singh</h3>
                              <span class="post">Amity University</span>                                      
                              <span class="d-block country_name"> Noida India</span>                               
                           </div>
                        </div>
                        <div class="col-md-12">
                           <div class="title_spekaer">
                              <h2>General Secretary</h2>
                           </div>
                        </div>
                        <div class="col-sm-12 col-md-3 col-lg-3">
                           <div class="our-team mt-3">
                              <div class="pic">
                                 <img src="assets/img/team/Ivanenko-Liudmyla-mam.jpg">
                              </div>
                              <h3 class="title">Prof.(Dr.) Ivanenko Liudmyla</h3>
                              <!-- <span class="post">Amity University</span>-->
                              <span class="d-block country_name"> Ukraine</span>                               
                           </div>
                        </div>
                        <div class="col-md-12">
                           <div class="title_spekaer">
                              <h2>Finance  Secretary</h2>
                           </div>
                        </div>
                        <div class="col-sm-12 col-md-3 col-lg-3">
                           <div class="our-team mt-3">
                              <div class="pic">
                                 <img src="assets/img/team/user.jpg">
                              </div>
                              <h3 class="title">Prof.(Dr.) Vinod Kumar</h3>
                              <!-- <span class="post">Amity University</span>-->
                              <span class="d-block country_name"> India</span>                               
                           </div>
                        </div>
                     </div>
                  </div>
                  <div class="tab-pane fade" id="working-committee">
                     <div class="row justify-content-center align-items-center clearfix">
                        <div class="col-sm-12 col-md-3 col-lg-3">
                           <div class="our-team mt-3">
                              <div class="pic">
                                 <img src="assets/img/team/dr-narendra-kumar.jpg">
                              </div>
                              <h3 class="title">Dr. Narendra Kumar (Member, Planning)</h3>
                              <span class="post">NIMS University, Jaipur</span>
                              <span class="d-block country_name">India</span>                               
                           </div>
                        </div>
                        <div class="col-sm-12 col-md-3 col-lg-3">
                           <div class="our-team mt-3">
                              <div class="pic">
                                 <img src="assets/img/team/ilona.jpg">
                              </div>
                              <h3 class="title">Prof.(Dr.) Ilona Paweloszek (Member, Planning)</h3>
                              <span class="post"></span>
                              <span class="d-block country_name">Poland</span>                               
                           </div>
                        </div>
                        <div class="col-sm-12 col-md-3 col-lg-3">
                           <div class="our-team mt-3">
                              <div class="pic">
                                 <img src="assets/img/team/user.jpg">
                              </div>
                              <h3 class="title">Prof.(Dr.) Gulsun Kurubacak  (Member, Planning)</h3>
                              <span class="post">Anadolu University</span>
                              <span class="d-block country_name">Turkey</span>                               
                           </div>
                        </div>
                        <div class="col-sm-12 col-md-3 col-lg-3">
                           <div class="our-team mt-3">
                              <div class="pic">
                                 <img src="assets/img/team/user.jpg">
                              </div>
                              <h3 class="title">Prof.(Dr.) Liliana Guran Manciu (Member, Planning)</h3>
                              <span class="post"></span>
                              <span class="d-block country_name">Romania </span>                               
                           </div>
                        </div>
                     </div>
                  </div>
               </div>
            </div>
         </div>
      </div>
   </div>
   </div>
</section>
<?php include('footer.php'); ?>
<?php include('script.php'); ?>